async function getSuggestedBids(
  size: number,
...
const { highRisk, midRisk, lowRisk } = await getSuggestedBids(
  748000,
...
console.log({
  highRisk: ethers.formatEther(highRisk),
  midRisk: ethers.formatEther(midRisk),
  lowRisk: ethers.formatEther(lowRisk),
});
